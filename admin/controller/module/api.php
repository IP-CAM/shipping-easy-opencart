<?php
class ControllerModuleApi extends Controller {
	private $error = array(); 

	public function index() {   
		$this->language->load('module/api');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('hello', $this->request->post);		
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/hello')) {
...
public function install() {
 ...
 ini_set("display_errors", 1);
		$this->db->query("CREATE TABLE `" . DB_PREFIX . "api1` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
                                PRIMARY KEY (`id`),
				`api` varchar(255) NOT NULL,
        `secretkey` varchar(255) NOT NULL,
        `baseurl` varchar(255) NOT NULL,
        `storeapi` varchar(255) NOT NULL
